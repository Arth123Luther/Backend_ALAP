...
describe ('Teste do recurso /tarefas', () => {
    ...
    describe ('PUT', () => {
        ...
        test('PUT /id deve retornar 422 (Vazio)', async() => {
            const response = await request.put(`${url}/${id}`).send({ nome: "   " });
            expect(response.status).toBe(422);
            expect(response.body.msg).toBe("Nome da tarefa é obrigatorio");
        });

        test('PUT /id deve retornar 422 (Curto)', async() => {
